...
# Chapter 9. Wiring a phone into Linux machines

> **The payoff of this chapter:** move files between a phone and a Linux
> laptop in one tap with LocalSend, the app I use on an iPhone, reach
> your Pi the same way from anywhere over Tailscale, know when KDE
> Connect’s clipboard sync is worth adding (and when it isn’t), and have
> the one wired fallback (USB) for when Wi-Fi isn’t an option.

The homelab is now reachable from mobile devices. This part closes the
loop in the *other* direction: getting a phone and Linux machines to
talk to each other for the everyday stuff, “send me that PDF,” “paste
this URL onto my desktop,” “pull these photos off the phone.” On a Mac,
AirDrop and Handoff are the obvious defaults; on Linux you assemble the
same conveniences from two small apps, plus one wired option you’ll
rarely need.
...
> that works no matter what a generic comparison table claims. That
> leaves the iOS KDE Connect app doing just file transfer, clipboard
> sync, and battery status, in a clunkier package than LocalSend. For
> the job almost everyone has on a phone, moving files, **LocalSend is
> simpler, nicer-looking, more reliable in day-to-day use, and all you
> need.** Reach for KDE Connect only if you specifically want clipboard
> sync or battery on your desktop and don’t mind the extra setup; on an
> iPhone it’s optional, and plenty of people (me included) skip it. One
> more iOS quirk if you do run it: Apple’s background limits mean the
> KDE Connect app must be **open or recently used** to stay reachable,
> so it can’t sit dormant for days and still answer.

### LocalSend: your AirDrop replacement

LocalSend is focused: open it, pick a nearby device, send the file. No
...
purely “get this file from here to there.” The connection finds the
other device and the transfer goes through more consistently than KDE
Connect’s does, and, just as important on an iPhone, **everything the
app offers works**, nothing greyed out, Android-only, or quietly broken.
That reliability is why I lean on it.

### KDE Connect: the optional companion

KDE Connect is the multitool, and on Android it is powerful. On an
iPhone its reach is narrower: the one feature worth having is the
**shared clipboard** (copy `192.168.1.50` on the phone, paste it on the
desktop), with **battery status** on your desktop panel as a minor
extra. The notification and SMS superpowers are Android-only, so on iOS
that’s the whole story, and whether it’s worth a second, fiddlier app
over just using LocalSend is a real question, answered next.

### Do you need KDE Connect on an iPhone?

Probably not. LocalSend already covers the thing most people do
constantly, moving files, and it does it with a cleaner app and zero
pairing. The only reason to add KDE Connect on an iPhone is if you want
**clipboard sync** between phone and desktop often enough to justify a
second, clunkier app. If that’s you, install both and let KDE Connect
handle the clipboard while LocalSend handles files. If it isn’t, install
LocalSend and stop there, you won’t miss anything. (On Android the math
flips: KDE Connect’s notification and SMS features make it well worth
running alongside LocalSend.)

## Install them

### On the iPhone

Install **LocalSend** from the App Store, that’s the app this chapter
starts with. If you also want clipboard sync, grab **KDE Connect** too
(the KDE Connect one is published by KDE e.V.); both are free.

### On your Linux laptop

**LocalSend** ships a proper Debian package, so on Ubuntu/Debian the
direct install is to grab the `.deb` from
[localsend.org/download](https://localsend.org/download) and hand it to
`apt`, which pulls in any dependencies and wires it into your normal
updates:
...
The `./` matters: it tells `apt` to install that local file rather than
look up a package by name. On Arch, install `localsend-bin` from the
AUR. (A distro-agnostic AppImage is also on the [download
page](https://localsend.org/download) if you’d rather not install
anything.)
...
- **LocalSend:** no pairing, open it on both devices, on the same Wi-Fi,
  and they appear in each other’s list. Send.
...
**LAN multicast/broadcast discovery**, great at home, useless the moment
your phone is on cellular, because broadcast packets don’t traverse a
VPN. But your tailnet gives every device a stable `100.x.y.z` address,
and both apps let you **add a device by IP manually**, which sidesteps
discovery entirely:
...
fit there. For moving a file between a phone and the Pi, you already
...
- **Drop it into a service you already run.** Files you want *on* the Pi
  ...
  `~/audiobookshelf/media/Audiobooks`, `scp` them over the tailnet:
...
stack**, and most people rarely need it. Install it when one of these
comes up, not before:
...
1.  **LocalSend**, the AirDrop replacement that handles the file job
    everyone has, and whose features work reliably.

Add **KDE Connect** only if you find yourself wanting clipboard sync
between phone and desktop; on Android, add it from the start, where its
notification and SMS features earn their keep. Either way, set up the
Tailscale manual-IP pairing once you want these to work away from home.
Then, *if* months later you find yourself wanting to pull a whole camera
roll off over a cable, install `libimobiledevice` + `ifuse` at that
point, when you’ll know exactly why you need them, instead of carrying
...
- **LocalSend** = start here: dead-simple file send with a clean app
  whose features work reliably, install it on the example iPhone and the
  laptop and you’re set for the common case.
- **KDE Connect** = optional companion: handy for clipboard sync and
  battery, but on an iPhone it’s limited (notifications/SMS are
  Android-only) and clunkier, so add it only if you want clipboard sync,
  it earns its place mainly on Android.
...
- **USB (`libimobiledevice` + `ifuse`)** is the wired fallback for bulk
  photo offload or no-Wi-Fi emergencies, install it only when needed.
...
privacy on the road, and now two-way file sharing with a phone. Every
piece runs on hardware under the reader’s control, over one private

...
> Linux laptop in one tap with LocalSend, the one app that’s all you
> need on an iPhone, reach your Pi the same way from anywhere over
> Tailscale, know when KDE Connect’s clipboard sync is worth adding (and
> when it isn’t), and have the one wired fallback (USB) for when Wi-Fi
> isn’t an option.
...
## The two Wi-Fi tools (and why LocalSend is the one to start with)
...
> [!IMPORTANT]
>
> ### On an iPhone, LocalSend is the one to reach for
>
...
> sync, and battery status, in a clunkier, less polished package than
> LocalSend. For the job almost everyone actually has on a phone, moving
> files, **LocalSend is simpler, nicer-looking, more reliable in
> day-to-day use, and all you need.** Reach for KDE Connect only if you
> specifically want clipboard sync or battery on your desktop and don’t
> mind the extra setup; on an iPhone it’s genuinely optional, and plenty
> of people (me included) skip it. One more iOS quirk if you do run it:
> Apple’s background limits mean the KDE Connect app must be **open or
> recently used** to stay reachable, so it can’t sit dormant for days
> and still answer.
...
purely “get this file from here to there.” It also tends to *just work*:
the connection finds the other device and the transfer goes through more
consistently than KDE Connect’s does, and, just as important on an
iPhone, **everything the app offers actually works**, nothing greyed
out, Android-only, or quietly broken. That reliability is a big part of
why it’s the one to lean on.

### KDE Connect: the optional companion

KDE Connect is the multitool, and on Android it’s genuinely powerful. On
an iPhone its reach is narrower: the one feature worth having is the
**shared clipboard** (copy `192.168.1.50` on your phone, paste it on
your desktop), with **battery status** on your desktop panel as a minor
...
### Do you actually need KDE Connect on an iPhone?

Honestly, probably not. LocalSend already covers the thing you do
...
Install **LocalSend** from the App Store, that’s the one you’ll actually
use. If you also want clipboard sync, grab **KDE Connect** too (the KDE
Connect one is published by KDE e.V.); both are free.

### On your Linux laptop

**LocalSend** ships a proper Debian package, so on Ubuntu/Debian the
cleanest install is to grab the `.deb` from
...
1.  **LocalSend**, the AirDrop replacement that handles the file job
    everyone actually has, and whose features all reliably work.
...
- **LocalSend** = the one to start with: dead-simple file send with a
  clean app whose features all actually work, install it on the iPhone
  and the laptop and you’re set for the common case.

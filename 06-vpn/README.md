...
- **Cost:** about **\$5/month**. Tailscale describes this as 5
  ...
  marketing page and its docs state the device math slightly
  differently, confirm the exact count at the checkout screen before
  buying.)
...
- **Aura:** part of the Aura identity-protection suite. A closed
  ...
  its supported platforms. It’s fine for casual “make me look like I’m
  elsewhere.” Its limitations for *this* project: it’s app-only (there’s
  no client you can run on the headless Pi, and no WireGuard config
  files to reuse), and like any full-device VPN it takes over DNS while
  it’s on.
- **Mullvad app:** a flat **€5/month** (~\$5.40), up to 5 devices,
  ...
  **WireGuard config files** you can run anywhere, including on the Pi.
  That openness is what makes the advanced setups in Part 7 possible.
...
- **What it’s good for:** routing through your **home** Pi so you appear
  ...
  your traffic on your own network where **Pi-hole still applies**.

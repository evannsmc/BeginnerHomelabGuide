...
and it’s worth understanding even if (like me) you don’t use it yet.
...
The key insight: because an exit node *is* still Tailscale, choosing one
...
`★ Insight ─────────────────────────────────────` This is the honest
trade at the center of the chapter. The add-on gives you *convenience*
(one tunnel, always on, homelab intact). Owning your own Mullvad sub
gives you *control* (portable configs, full settings, no extra account
you don’t manage). Right now I value control more, and I’m fine
switching apps by hand to get it. If my needs shift toward “always-on
privacy that never costs me the homelab,” I’ll pay the extra and run
both, or move fully to the add-on. It’s a budget-and-priorities call,
...
|----|----|----|----|----|
| **Homelab + ad-blocking** (my everyday default) | Tailscale on, no exit node | ✅ via DNS push | ✅ | ❌ (your real IP) |
| **Privacy, my way** (what I run) | Tailscale off, **standalone Mullvad on** | ❌ Mullvad’s DNS | ❌ | ✅ |
| **Appear at home / reach LAN** | Tailscale + **Pi as exit node** (Flavor 2) | ✅ | ✅ | ❌ (home IP) |
| **Privacy *and* homelab at once** | **Mullvad add-on exit node** (Flavor 1) | ❌ Mullvad’s DNS | ✅ | ✅ |

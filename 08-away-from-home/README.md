...
- **The Mullvad exit node (Option A) is the only privacy choice that
  keeps your homelab reachable**, because it *is* Tailscale, your tunnel
  to `homelab` stays up while Mullvad handles egress. It does, however,
  hand DNS to Mullvad, so Pi-hole is bypassed (use Mullvad’s own
  ad-blocking DNS as a substitute).
...
- **The governing rule:** a full-device VPN owns DNS, and a phone runs
  one VPN at a time, so homelab access, Pi-hole filtering, and a privacy
  VPN can’t all be on at once. Switch between modes using the decision
  matrix.
